...
\textbf{Case 1:} $\sigma = x$.

Then $\sigma(x/\tau) = \tau$ and
\begin{equation*}
\sigma^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = (j\cfrac{\tau^{(\M, j)}}{x})(x) = \tau^{\M, j} = (\sigma(x/\tau))^{\M, j}.
\end{equation*}

\textbf{Case 2:} $\sigma = v$ where $v\in\V$ and $v\neq x$.

Then $\sigma(x/\tau) = v$ and
\begin{equation*}
\sigma^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = (j\cfrac{\tau^{(\M, j)}}{x})(v) = j(v) = v^{\M, j} = (\sigma(x/\tau))^{\M, j}.
\end{equation*}

\textbf{Case 3:} $\sigma = c$ where $c\in\mathscr{F}_0$.

Then $\sigma(x/\tau) = c$ and
\begin{equation*}
\sigma^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = c^\M = (\sigma(x/\tau))^{\M, j}.
\end{equation*}

\textbf{Case 4:} $\sigma = f(\sigma_1, \dots, \sigma_k)$ where $f\in\mathscr{F}_k$ and $\sigma_1, \dots, \sigma_k$ are terms.

Then $\sigma(x/\tau) = f(\sigma_1(x/\tau), \dots, \sigma_k(x/\tau))$. By inductive hypothesis, $\sigma_i^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = (\sigma_i(x/\tau))^{\M, j}$ for all $i = 1, \dots, k$. Thus
\begin{align*}
...
By the definition of substitution in well formed formula, $\phi(x/\tau) = p(\sigma_1(x/\tau), \dots, \sigma_k(x/\tau))$. By Lemma \ref{term-substitution-lemma}, for each term $\sigma_i$, we have $\sigma_i^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = (\sigma_i(x/\tau))^{\M, j}$. Therefore,
\begin{align*}
(\M, j\cfrac{\tau^{(\M, j)}}{x})\models \phi &\iff \phi^{\M, j\cfrac{\tau^{(\M, j)}}{x}} = 1 \\
