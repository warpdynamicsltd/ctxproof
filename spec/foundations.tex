...
on $\L$ iff $\M_1(\MM \cup \FF) = \M_2(\MM \cup \FF)$. We will denote it by $M'|_\L = M|_\L$.
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $k$ be a non-negative integer, let $q\in \mathscr{P}^g_k$ and $\bm{x}\in \dis{\V}{k}$. Then $\text{Red\,}_\L^{q(\bm{x})}$ is weakly truth-preserving.
\end{theorem}

\begin{proof}
Take any $\L^G$-structure $\M$ and any $\gamma\in \F(\L^G)$ such that $\M'\models \gamma$ for all $\L^G$-structure $\M'$ for which $\M|_\L = \M'|_\L$.

We need to show that $\M\models \text{Red\,}_\L^{q(\bm{x})}(\gamma)$.
...
Therefore, $\M\models \text{Red\,}_\L^{q(\bm{x})}(\gamma)$.

\textbf{Case 2:} Otherwise.

Then $\text{Red\,}_\L^{q(\bm{x})}(\gamma) = \truth$, which is trivially satisfied by any structure.

Therefore, $\text{Red\,}_\L^{q(\bm{x})}$ is weakly truth-preserving.
\end{proof}
