...
Let $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $\Gamma \subseteq \F(\L)$ and $\phi\in\F(\L)$. We define that $\Gamma \models \phi$ iff for any non-empty domain $A$ and any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$, if $\M, j\models \psi$ for all $\psi\in\Gamma$, then $\M, j\models \phi$.
\end{definition}
...
Take any $\L$-structure with domain $A$.
...
\label{modus-ponens}
Let $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $\Gamma \subseteq \F(\L)$. Let $\alpha, \beta\in\F(\L)$. If $\Gamma\models \alpha$ and $\Gamma\models \alpha \to \beta$, then $\Gamma\models \beta$.
\end{theorem}

\begin{proof}
Assume $\Gamma\models \alpha$ and $\Gamma\models \alpha \to \beta$.

Take any non-empty domain $A$ and any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$. We need to show that $\M, j\models \beta$.

Since $\Gamma\models \alpha$ and $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we have $\M, j\models \alpha$, which means $\alpha^{\M, j} = 1$.

Since $\Gamma\models \alpha \to \beta$ and $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we have $\M, j\models \alpha \to \beta$, which means $(\alpha \to \beta)^{\M, j} = 1$.
...
Therefore, $\M, j\models \beta$. Since this holds for any non-empty domain $A$, any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we conclude that $\Gamma\models \beta$.
\end{proof}

\begin{theorem}[\textbf{generalization}]
\label{generalization}
Let $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $\Gamma \subseteq \F(\L)$. Let $v\in\V$ and $\phi\in\B(\L)$. If $v\not\in\free(\gamma)$ for all $\gamma\in\Gamma$ and $\Gamma\models \phi$, then $\Gamma\models \forall v. \phi$.
\end{theorem}

\begin{proof}
Assume $v\not\in\free(\gamma)$ for all $\gamma\in\Gamma$ and $\Gamma\models \phi$.

Take any non-empty domain $A$ and any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$. We need to show that $\M, j\models \forall v. \phi$.
...
We need to show that $\phi^{\M, j\cfrac{a}{v}} = 1$ for all $a\in A$.

Take any $a\in A$. Since $v\not\in\free(\gamma)$ for all $\gamma\in\Gamma$, by Theorem \ref{signature-of-formula}, we have $\gamma^{\M, j} = \gamma^{\M, j\cfrac{a}{v}}$ for all $\gamma\in\Gamma$.

Therefore, $\M, j\cfrac{a}{v}\models \gamma$ for all $\gamma\in\Gamma$.

Since $\Gamma\models \phi$ and $\M, j\cfrac{a}{v}\models \gamma$ for all $\gamma\in\Gamma$, we have $\M, j\cfrac{a}{v}\models \phi$, which means $\phi^{\M, j\cfrac{a}{v}} = 1$.
...
Hence $\M, j\models \forall v. \phi$. Since this holds for any non-empty domain $A$, any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we conclude that $\Gamma\models \forall v. \phi$.
\end{proof}

\begin{theorem}
\label{modus-ponens-from-def}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $q\in \mathscr{P}$. Let $\bm{x}\in \dis{\V}{k}$. Let $\Gamma\subset \F(\L)$ and $\psi, \phi\in \F(\L)$, such that $q$ does not occur in any formula from the set $\Gamma \cup \{\psi, \phi\}$. Then
\begin{equation}
...
Take any non-empty domain $A$ and any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$. We need to show that $\M, j\models \psi$.

Since $\Gamma\models (q(\bm{x}) \liff \phi) \to \psi$ and $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we have
\begin{equation}
...
By the definition of formula evaluation for implication,
\begin{equation}
((q(\bm{x}) \liff \phi) \to \psi)^{\M, j} = 1 - (q(\bm{x}) \liff \phi)^{\M, j}(1 - \psi^{\M, j}) = 1.
\end{equation}

Now we will modify the structure $\M$ to create a new structure $\M'$ that differs only in the interpretation of $q$. Since $q$ does not occur in any formula from $\Gamma \cup \{\psi, \phi\}$, we can define $\M'$ such that for any $\bm{a}\in A^k$:
\begin{equation}
...
Since $q$ does not occur in any formula from $\Gamma$, for any $\gamma\in\Gamma$ we have $\gamma^{\M, j} = \gamma^{\M', j}$. Therefore, $\M', j\models \gamma$ for all $\gamma\in\Gamma$.

With the structure $\M'$, we have
\begin{equation}
q(\bm{x})^{\M', j} = (q)^{\M'}(\bm{x}^{\M', j}) = \phi^{\M, j\cfrac{\bm{x}^{\M', j}}{\bm{x}}}.
\end{equation}
Since by Definition \ref{term-eval-def}, we have $\bm{x}^{\M', j} = \bm{x}^{\M, j}$, thereforre
...
Hence $q(\bm{x})^{\M', j} = \phi^{M, j}$ and this by Theorem \ref{biconditional-evaluation} means that $(q(\bm{x}) \liff \phi)^{\M', j} = 1$.


Now, since $\Gamma\models (q(\bm{x}) \liff \phi) \to \psi$ and $\M', j\models \gamma$ for all $\gamma\in\Gamma$, we have
\begin{equation}
...
Since $(q(\bm{x}) \liff \phi)^{\M', j} = 1$, we have
\begin{equation}
1 - 1 \cdot (1 - \psi^{\M', j}) = 1,
\end{equation}
which implies $\psi^{\M', j} = 1$. And since $q$ does not occur in $\psi$, $\psi^{\M, j} = 1$, so $\M, j\models \psi$.

Since this holds for any non-empty domain $A$, any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ such that $\M, j\models \gamma$ for all $\gamma\in\Gamma$, we conclude that $\Gamma\models \psi$.
\end{proof}
