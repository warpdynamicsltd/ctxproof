...
Let $\Gamma\subset\F(\L)$ such that for any $\gamma\in \Gamma$ and for any $\L$-structure $\K$ with domain $A$
\begin{equation}
\label{modus-ponens-from-def-q-arbitrary}
\K,j\models \gamma \implies \K',j\models\gamma \text{ for all $\K'\in \MM_q(\K)$}.
\end{equation}

If for any $\L$-structre $\M$ with domain $A$ such that $\M, j\models \Gamma$, we have 
...
Since $\M'\in\MM_q(M)$ and by (\ref{modus-ponens-from-def-q-arbitrary}), we have $\M', j\models \Gamma$ and thus by assumption (\ref{modus-ponens-from-def-assumption}), we have $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$.
...
\begin{equation}
\label{modus-ponens-from-def-q-arbitrary-v2}
\K \models \gamma \implies \K'\models\gamma \text{ for all $\K'\in \MM_q(\K)$}.
\end{equation}

Then
\begin{equation}
\Gamma\models (q(\bm{x}) \liff \phi) \to \psi \implies \Gamma\models \psi.
\end{equation}

\end{lemma}

\begin{proof}
Assume $\Gamma\models (q(\bm{x}) \liff \phi) \to \psi$.
...
By condition (\ref{modus-ponens-from-def-q-arbitrary-v2}), since $\M\models \gamma$ for all $\gamma\in\Gamma$ and $\M'\in\MM_q(\M)$, we have $\M'\models \gamma$ for all $\gamma\in\Gamma$. Therefore, $\M'\models \Gamma$.

Since $\Gamma\models (q(\bm{x}) \liff \phi) \to \psi$ and $\M'\models \Gamma$, we have $\M'\models (q(\bm{x}) \liff \phi) \to \psi$.
