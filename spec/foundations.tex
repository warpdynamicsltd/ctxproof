...
Since $\M'\in\MM_q(M)$ and by (\ref{modus-ponens-from-def-q-arbitrary}), we have $\M', j\models \Gamma$ and thus by assumption (\ref{modus-ponens-from-def-assumption}), we have $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$, which is the same as
\begin{equation}
...
Let  $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and $A$ be an arbitrary non-empty mathematical domain. Let $\phi_1, \phi_2, \psi_1, \psi_2\in \F(\L)$ such that $\rep{\phi_1}{\phi_2}{\psi_1}{\psi_2}$. Let $\M$ be an $\L$-structure with domain $A$. If $M, j\models \phi_1 \liff \phi_2$ for any $j:\V\to A$,
then $M, j\models \psi_1 \liff \psi_2$ for any $j:\V\to A$.
\end{theorem}
