...
Let $\L$ be a signature of a language and $A$ be an arbitrary mathematical domain. Let $\M$ be an $\L$-structure with domain $A$ and $j:\V\to A$ be an assignment. 
...
Let $\L$ be a signature of a language. Let $\phi\in\F(\L)$.
Let $k$ be a positive integer and
let $\bm{x}\in \dis{\V}{k}$ and $\bm{\tau}\in\T(\L)^k$.
...
\quad 
\big(\text{by choice of $\bm{v}$, $j\cfrac{\bm{\tau}^{\M, j}}{\bm{x}}\cfrac{a}{u}$ and 
$j\cfrac{\bm{\sigma}^{\M, j}}{\bm{v}}\cfrac{a}{u}$ agrees on $\free(\psi)$}\big)\\
