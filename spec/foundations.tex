...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $q\in \mathscr{P}$. Let $\bm{x}\in \dis{\V}{k}$. Let $\Gamma\subset \F(\L)$ and $\psi, \phi\in \F(\L)$, such that $q$ does not occure in any formula from the set $\Gamma \cup \{\psi, \phi\}$. Then
\begin{equation}
