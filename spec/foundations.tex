...
For any $z\in \mathscr{P}\cup\mathscr{F}$,
\begin{equation}
\begin{aligned}
\MM_z(\M) \defeq \{\M': \M' 
\text{is $\L$-structure with domain $A$} \\
\text{and $\M'(s) = \M(s)$ for all $s\in(\mathscr{P}\cup \mathscr{F})\setminus \{z\}$ }\}. 
\end{aligned}
\end{equation}
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language. Let $k$ be a non-negative integer, let $q\in \mathscr{P}_k$ and $\bm{x}\in \dis{\V}{k}$. Let $\phi, \psi\in\F(\L)$ such that $q$ does not occure in both $\phi$ and $\psi$.

Let $A$ be an non-empty mathematical domain and $j:\V\to A$.
Let $\M$ be $\L$-structure with domain $A$. Then

\begin{equation}
\label{modus-ponens-from-def-implication}
\M',j\models (q(\bm{x}) \liff \phi) \to \psi \text{ for any } \M'\in\MM_q(\M) \implies \M,j\models \psi.
\end{equation}

\end{lemma}

\begin{proof}
Assume that assumption of implication (\ref{modus-ponens-from-def-implication}) holds. 
We will construct a new structure $\M'$ that differs from $\M$ only in the interpretation of $q$. Namely, 
\begin{equation}
(q)^{\M'}(\bm{a}) = \phi^{\M, j\cfrac{\bm{a}}{\bm{x}}}.
\end{equation}

For all other predicates and functions, $\M'$ agrees with $\M$. Note that $\M'\in\MM_q(\M)$. 
...
we have $\phi^{\M, j} = \phi^{\M', j}$ and consequently $q(\bm{x})^{\M', j} = \phi^{\M', j}$. By Theorem \ref{biconditional-evaluation} this is the same as $(q(\bm{x}) \liff \phi)^{\M', j} = 1$, and by Definition \ref{interpretation-def}, we have $\M', j\models q(\bm{x}) \liff \phi$.


By assumption of implication (\ref{modus-ponens-from-def-implication}), since $\M'\in\MM_q(\M)$, we have $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$.
And then, since $\M', j\models q(\bm{x}) \liff \phi$, by Theorem \ref{modus-ponens} we have $\M', j\models \psi$.

And since $q$ does not occur in $\psi$, we have $\psi^{\M', j} = \psi^{\M, j}$, therefore $\M, j\models \psi$.
\end{proof}

\begin{lemma}
\label{modus-ponens-from-def-v2}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language. Let $k$ be a non-negative integer, let $q\in \mathscr{P}_k$ and $\bm{x}\in \dis{\V}{k}$. Let $\phi, \psi\in\F(\L)$ such that $q$ does not occur in both $\phi$ and $\psi$.
...
\begin{proof}
Assume that assumption of implication (\ref{modus-ponens-from-def-v2-implication}) holds. Take an arbitrary $\L$-structure $\M$ with domain $A$.
Take an arbitrary assignment $j:\V\to A$. Take an arbitrary $\M'\in\MM_q(\M)$.
By assumption from implication (\ref{modus-ponens-from-def-v2-implication}), we have $\M'\models (q(\bm{x}) \liff \phi) \to \psi$. Thus in particular $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$, but since $\M'$ is arbitrary chosen, by Theorem \ref{modus-ponens-from-def}, we have 
$\M, j\models \psi$. And since $j$ was arbitrary chosen, we have $\M\models\psi$.
\end{proof}
...
\item If $\psi_1 \sxeq p(\bm{\eta})$ and $\psi_2 \sxeq p(\bm{\eta})$ where $\bm{\eta}\in\T(\L)^*$ and $p\in \mathscr{P}_{|\bm{\eta}|}$, then $\rep{\psi_1}{\phi_2}{\psi_1}{\phi_2}$ holds. 
...
Let  $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and $A$ be an arbitrary non-empty mathematical domain. Let $\phi_1, \phi_2, \psi_1, \psi_2\in \F(\L)$ such that $\rep{\phi_1}{\phi_2}{\psi_1}{\psi_2}$. For any $\L$-structure $\M$,
\begin{equation}
