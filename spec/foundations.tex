...
\M'\models \psi(x/c) \text{ for any }  \M'\in\MM_q(\M) \implies \M\models \psi.
\end{equation}
...
\begin{definition}
Let $\L = (\FF, \PP, \arity)$ be signature of a language and $\FF^g \cap \FF = \emptyset$ and $\PP^g \cap \PP = \emptyset$. We will call such $\L$ a sub-signature and consider extention of the language with signature
\begin{equation}
\L^G \defeq (\FF^g \cup \FF, \PP^g \cup \PP, \arity \cup \arity^g). 
\end{equation}
\end{definition}

When we consider $\L^G$-structures $\M_1, \M_2$ and $\L^G$-structure $M'$, we say that they agree 
...
\begin{definition}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language. Let $k_1, k_2$ be non-negative integer. We will say that $r$ is an inference rule iff $r: \F(\L)^{k_1} \to\F(\L)^{k_2}$.
\end{definition}
...
\begin{definition}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $k_1, k_2$ be non-negative integers and let $r: \F(\L)^{k_1} \to\F(\L)^{k_2}$, we will say that $r$ is \textbf{weakly truth-preserving}
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $k_1, k_2$ be non-negative integers and let $r: \F(\L)^{k_1} \to\F(\L)^{k_2}$. If $r$ is strictly truth-preserving, then $r$ is weakly truth-preserving.
\end{corollary}
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $k$ be a non-negative integer, let $q\in \mathscr{P}^g_k$ and $\bm{x}\in \dis{\V}{k}$. Then $\text{Red\,}_{\L^G}^{q(\bm{x})}$ is weakly truth-preserving.
\end{theorem}
...
\begin{definition}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $\R$ be a family of inference rules. Let $\Gamma\subset \F(\L)$. We will say that $\Gamma$ is closed with respect to $\R$ iff for any $r\in \R$ with $\dim r = (k_1, k_2)$, we have $r(\Gamma^{k_1}) \subset \Gamma^{k_2}$. 
\end{definition}

\begin{definition}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language. Let $\R$ be a family of inference rules. Let $T \subset \F(\L^G)$.
\begin{equation}
...
\begin{definition}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language.
\end{definition}
\begin{equation}
\G(\L^G) \defeq \{\phi\in\F(\L^G): 
\begin{aligned}
&\M\models \phi \implies \M'\models \phi 
\text{ for any $\M'$ for which $\M|_\L = \M'|_\L$}, \\
&\text{ for any $\L^G$-structure $\M$. }
\end{aligned}
\}
\end{equation}
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature of a language.
Let $\R$ be a family of weakly truth-preserving inference rules. If $T\subset \G(\L^G)$, then $\Cn(T, \R)\subset \G(\L^G)$.
\end{theorem}

\begin{proof}
We will show that $\G(\L^G)$ is closed with respect to $\R$ and $T\subset \G(\L^G)$. Then by definition of $\Cn(T, \R)$, we have $\Cn(T, \R)\subset \G(\L^G)$.

Since $T\subset \G(\L^G)$ by assumption, it remains to show that $\G(\L^G)$ is closed with respect to $\R$.

Take any $r\in \R$ with $\dim r = (k_1, k_2)$. We need to show that $r(\G(\L^G)^{k_1}) \subset \G(\L^G)^{k_2}$.

Take any $\bm{\phi}\in \G(\L^G)^{k_1}$ and any $\L^G$-structure $\M$ such that $\M\models r(\bm{\phi})$.

We need to show that $r(\bm{\phi})\in \G(\L^G)^{k_2}$, which means for each component $\psi$ of $r(\bm{\phi})$, we need to show: if $\M\models \psi$ then $\M'\models \psi$ for any $\M'$ with $\M|_\L = \M'|_\L$.

Take any component $\psi$ of $r(\bm{\phi})$ and suppose $\M\models \psi$ for some $\L^G$-structure $\M$.

Take any $\L^G$-structure $\M'$ with $\M|_\L = \M'|_\L$.

Since $\bm{\phi}\in \G(\L^G)^{k_1}$, each component of $\bm{\phi}$ is in $\G(\L^G)$.

Therefore, for any $\L^G$-structure $\M''$ with $\M|_\L = \M''|_\L$, we have $\M''\models \bm{\phi}$.

Since $r$ is weakly truth-preserving and $\M''\models \bm{\phi}$ for all $\M''$ with $\M|_\L = \M''|_\L$, we have $\M\models r(\bm{\phi})$.

In particular, $\M\models \psi$ where $\psi$ is a component of $r(\bm{\phi})$.

By the same argument applied to $\M'$ (since $\M'|_\L = \M|_\L$), we have $\M'\models r(\bm{\phi})$, hence $\M'\models \psi$.

Therefore $\psi\in \G(\L^G)$, and thus $r(\bm{\phi})\in \G(\L^G)^{k_2}$.
\end{proof}
\end{document}