...
Since $\M'\in\MM_q(M)$, we have $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$.
Since $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$ and $\M', j\models q(\bm{x}) \liff \phi$, by Theorem \ref{modus-ponens} we have $\M', j\models \psi$.

And since $q$ does not occur in $\psi$, we have $\psi^{\M', j} = \psi^{\M, j}$, therefore $\M, j\models \psi$.
\end{proof}

Lemma \ref{modus-ponens-from-def} can be re-formulated in a way which binds it more precisely with semantic consequence, but this is at expense of additional assumption that $\free(\phi)\subset \bm{x}$.

\begin{lemma}
\label{modus-ponens-from-def-v2}
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language. Let $k$ be a non-negative integer, let $q\in \mathscr{P}_k$ and $\bm{x}\in \dis{\V}{k}$. Let $\phi, \psi\in\F(\L)$ such that $q$ does not occure in both $\phi$ and $\psi$, and $\free(\phi)\subset \bm{x}$.
...
\begin{proof}
Assume that assumption of implication (\ref{modus-ponens-from-def-v2-implication}) holds. 

We will construct a new structure $\M'$ that differs from $\M$ only in the interpretation of $q$.
Fix some $j_0:\V\to A$. 
Define $\M'$ such that for any $\bm{a}\in A^k$:
\begin{equation}
(q)^{\M'}(\bm{a}) = \phi^{\M, j_0\cfrac{\bm{a}}{\bm{x}}}.
\end{equation}

For all other predicates and functions, $\M'$ agrees with $\M$. Note that $\M'\in\MM_q(\M)$.

By assumption we have $\M'\models (q(\bm{x}) \liff \phi) \to \psi$.

Take any assignment $j:\V\to A$. We need to show that $\M, j\models \psi$.

In particular, $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$.

Now we show that $\M', j\models q(\bm{x}) \liff \phi$.

By the construction of $\M'$, we have
\begin{equation}
q(\bm{x})^{\M', j} = (q)^{\M'}(\bm{x}^{\M', j}) = \phi^{\M, j_0\cfrac{\bm{x}^{\M', j}}{\bm{x}}}.
\end{equation}

Since by Definition \ref{term-eval-def}, $\bm{x}^{\M', j} = \bm{x}^{\M, j} = j(\bm{x})$, we have
\begin{equation}
q(\bm{x})^{\M', j} = \phi^{\M, j_0\cfrac{j(\bm{x})}{\bm{x}}}.
\end{equation}

Since $\free(\phi)\subset \bm{x}$, by Theorem \ref{signature-of-formula}, the assignment $j_0\cfrac{j(\bm{x})}{\bm{x}}$ and $j$ agree on $\free(\phi)$, so
\begin{equation}
\phi^{\M, j_0\cfrac{j(\bm{x})}{\bm{x}}} = \phi^{\M, j}.
\end{equation}

Since $q$ does not occur in $\phi$, we have $\phi^{\M, j} = \phi^{\M', j}$.

Therefore, $q(\bm{x})^{\M', j} = \phi^{\M', j}$, which by Theorem \ref{biconditional-evaluation} means $(q(\bm{x}) \liff \phi)^{\M', j} = 1$.

Hence $\M', j\models q(\bm{x}) \liff \phi$.

Since $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$ and $\M', j\models q(\bm{x}) \liff \phi$, by Theorem \ref{modus-ponens} we have $\M', j\models \psi$.

Since $q$ does not occur in $\psi$, we have $\psi^{\M', j} = \psi^{\M, j}$, therefore $\M, j\models \psi$.
...
Let $\Gamma\subset\F(\L)$ such that for any $\gamma\in \Gamma$ and for any $\L$-structure $\K$
\begin{equation}
\label{modus-ponens-from-def-c-arbitrary}
\K \models \gamma \implies \K'\models\gamma \text{ for all $\K'\in \MM_c(\K)$}.
\end{equation}

Then
\begin{equation}
\Gamma\models \psi(x/c) \implies \Gamma\models \psi.
\end{equation}

\end{lemma}

\begin{proof}
Assume $\Gamma\models \psi(x/c)$ where $\psi(x/c)$ is an admissible substitution.

Take any $\L$-structure $\M$ with domain $A$ such that $\M\models \Gamma$. We need to show that $\M\models \psi$.
...
By condition (\ref{modus-ponens-from-def-c-arbitrary}), since $\M\models \gamma$ for all $\gamma\in\Gamma$ and $\M'\in\MM_c(\M)$, we have $\M'\models \gamma$ for all $\gamma\in\Gamma$. Therefore, $\M'\models \Gamma$.

Since $\Gamma\models \psi(x/c)$ and $\M'\models \Gamma$, we have $\M'\models \psi(x/c)$.
...
Since this holds for any $\L$-structure $\M$ such that $\M\models \Gamma$, we conclude that $\Gamma\models \psi$.
\end{proof}
...
\begin{cases}
\psi_1 \liff \psi_2 \text{ for } \phi \sxeq \phi_1 \liff \phi_1 \text{ where }
...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a sub-signature. Let $k$ be a non-negative integer.

We will define a family of functions 
$\text{Red}_\L^{q(\bm{x})}: \F(\L^G)\to \F(\L)$ for $q\in \mathscr{P}^g_k$ and $\bm{x}\in \dis{\V}{k}$.

\begin{equation}
\text{Red}_\L^{q(\bm{x})}(\gamma) \defeq 
\begin{cases}
\psi \text{ for } \gamma \sxeq (q(\bm{x}) \liff \phi) \to \psi \text{ where } \psi, \phi\in \B(\L) \text{ and } \free(\phi)\subset \bm{x},\\
\truth \text{ otherwise.}
\end{cases}
\end{equation}
\end{definition}
