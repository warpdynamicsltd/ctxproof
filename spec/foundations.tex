...
Let $\L = (\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $\phi\in\F(\L)$.We define that $\models \phi$ iff for any non-empty domain $A$ and any $\L$-structure $\M$ with domain $A$, and any $j:\V\to A$ we have $\M, j\models \phi$.
\end{definition}
