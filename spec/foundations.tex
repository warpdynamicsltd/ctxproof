...
Let $\L=(\mathscr{P}, \mathscr{F}, \arity)$ be a signature of a language and let $q\in \mathscr{P}$. Let $\phi, \psi\in\F(\L)$ and $q$ occurse in neither $\phi$ nor $\psi$. Let $\bm{x}\in \dis{\V}{k}$. Let $A$ be any non-empty mathematical domain and $j:\V\to A$. If for any $\L$-structre $\M$ with domain $A$, we have 
$\M, j\models (q(\bm{x}) \liff \phi) \to \psi$, then for any $\L$-structre $\M$ with domain $A$, we have $\M, j\models \psi$.
\end{theorem}

\begin{proof}
Assume that for any $\L$-structre $\M$ with domain $A$, we have
\begin{equation}
\label{modus-ponens-from-def-assumption}
\M, j\models (q(\bm{x}) \liff \phi) \to \psi.
\end{equation}

Take an arbitrary $\L$-structre $\M$ with domain $A$. We will modify the structure $\M$ to create a new structure $\M'$ that differs only in the interpretation of $q$. Namely, 
\begin{equation}
...
Now, by assumption (\ref{modus-ponens-from-def-assumption}), we have $\M', j\models (q(\bm{x}) \liff \phi) \to \psi$, which is the same as
\begin{equation}
