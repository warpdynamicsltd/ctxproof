...
let rec string_of_term t =
  ...
  | Var v -> v
  | Const c -> c
  | SkolemConst lst -> "skolem _" ^ String.concat "_" (List.map string_of_int lst)
  | SkolemFunc (lst, args) -> "skolem _" ^ String.concat "_" (List.map string_of_int lst) ^ "(" ^ (String.concat ", " (List.map string_of_term args)) ^ ")"
  | Func (f, args) ->
