...
assert_eq __LINE__ statement_of_string "0 $true {R:MOD} {} {X, X};"
(Statement {ref=make_ref ["0"]; formula=True; statements=[||]; inference=Inference {mode=Rule "MOD"; gformulas=[]; terms=[Var "X"; Var "X"]}; pos=init_pos("")});
...
assert_eq __LINE__ statement_of_string "0 $true {R:MOD} {$true; 0.1} {X, f(c)};"
(Statement {ref=make_ref ["0"]; formula=True; statements=[||]; inference=Inference {mode=Rule "MOD"; gformulas=[Formula True; Reference (make_ref ["0"; "1"])]; terms=[Var "X";Func ("f", [Const "c"])]}; pos=init_pos("")});
